...
/// <summary>
/// Represents a relationship entity for a <see cref="LncEntity{T}"/>
/// </summary>
[Interface(typeof(IDbModel), nameof(Type), nameof(Data))]
[JsonConverter(typeof(InterfaceParser<LncRelationship>))]
public class LncRelationship
{
	/// <summary>
	/// The map of all of the interface options for mapping
	/// </summary>
	[JsonIgnore]
	public static InterfaceMap<LncRelationship> Map { get; } = new();
	...
	public static LncRelationship? FromEntity(IDbModel? model)
	{
		if (model is null) return null;

		var type = model.GetType();
		var name = Map.Map.FirstOrDefault(t => type == t.Map)
			?? throw new Exception($"{nameof(LncRelationship)} >> {type.Name} not mapped");

		return new LncRelationship
		{
			Type = name.Name,
			Data = model
		};
	...
	public static void Apply(List<LncRelationship> items, IDbModel? entity)
	{
	...
	public static void Apply(List<LncRelationship> items, IEnumerable<IDbModel?> entities)
	{
