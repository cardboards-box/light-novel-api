...
/// <summary>
/// Controllers for handling cover images
/// </summary>
...
{
	/// <summary>
	/// Fetches the meta-data for the given image
	/// </summary>
	/// <param name="id">The ID of the image</param>
	/// <returns>The meta-data of the image</returns>
	[HttpGet, Route("image/{id}/meta")]
	[Produces<LncCover>, ProducesError(404), ProducesError(400)]
	public Task<IActionResult> ImageData([FromRoute] string id) => Box(async () =>
	{
		if (!Guid.TryParse(id, out var guid))
			return Boxed.Bad($"Invalid image ID: {id}");

		var result = await _db.Cover.Fetch(guid);
		if (result is null)
			return Boxed.NotFound($"Image with ID {id} not found");
	...
	public async Task<IActionResult> Get([FromRoute] string id, CancellationToken token)
	{
		...
		var result = await _cache.Get(guid, token);
