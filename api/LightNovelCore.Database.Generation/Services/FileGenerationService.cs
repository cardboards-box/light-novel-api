namespace LightNovelCore.Database.Generation.Services;
...
internal interface IFileGenerationService
{
    ...
    Task OrmClass(TableEntity table, StreamWriter writer, int asOf, string nameSpace, Entities entities, string prefix);
    ...
    IDatabaseMetadataService _metadata) : IFileGenerationService
{
    ...
    public async Task OrmClass(TableEntity table, StreamWriter writer, int asOf, string nameSpace, Entities entities, string prefix)
    {
...
");
            readers.Add($"{prefix}Relationship.Apply(related, await rdr.ReadAsync<{parent.Type.Name}>());");
        }
...
");
            readers.Add($"{prefix}Relationship.Apply(related, await rdr.ReadAsync<{notEntity.Table.Type.Name}>());");
        }
...
");
                readers.Add($"{prefix}Relationship.Apply(related, await rdr.ReadAsync<{entity.Type.Name}>());");
            }
        }
    ...
    public async Task<{5}Entity<{0}>?> {1}(Guid id)
    {{
...
{2}"";
        ...
        return new {5}Entity<{0}>(item, [..related]);
    }}";
    ...
    Task<{prefix}Entity<{type}>?> {RELATIONSHIP_NAME}(Guid id);");
        }
